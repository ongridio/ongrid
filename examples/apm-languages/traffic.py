...
def run(language, port, check=False):
...
if __name__ == "__main__":
    with ThreadPoolExecutor(max_workers=4) as pool:
        jobs = [pool.submit(run, language, port, "--check" in sys.argv) for language, port in [("go", 18080), ("java", 18082), ("node", 18084), ("python", 18086)]]
        for job in jobs:
            job.result()
