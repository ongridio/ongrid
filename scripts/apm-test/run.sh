...
APM_TEST_PROMTOOL="$(pwd)/scripts/apm-test/promtool.sh" APM_TEST_PROMETHEUS=http://127.0.0.1:19090/prometheus APM_TEST_TEMPO=http://127.0.0.1:13200 APM_TEST_OTLP=http://127.0.0.1:14318/v1/traces GOCACHE="${GOCACHE:-/tmp/ongrid-go-build-cache}" go test -race -count=1 -run 'TestOTLPIntegration|TestClusterMigrationTraceQLIntegration|TestAlertDwellPromtool' -v ./internal/manager/biz/apm
