...
func (t *ConfigTool) Info(_ context.Context) (*basetool.ToolInfo, error) {
	...
	case configToolDraftChange:
		return &basetool.ToolInfo{
			...
			WhenToUse:   "Use when the user asks to configure, create, or add an alert rule from natural language. Only domain=alert_rule and action=create are supported. For metric-based requests, call list_metric_catalog once first and use returned metric names/sample_labels as the source of truth; use analyze_database_status only if database source/capability context is still missing. If the metric catalog is empty or lacks the required signal, do not invent metric names; ask for exact PromQL/metrics unless the user already supplied them, then call this tool and let draft validation decide whether the rule is confirmable. Do not call list_database_sources, query_knowledge, code tools, or AgentTool for alert-rule creation. This tool validates the candidate rule before issuing a confirmable draft. If it returns kind=config_validation_failed, read validation.issues, fix the rule arguments, and call draft_config_change again in the same user turn. A natural-language alert creation is not complete until this tool returns config_draft with draft_hash; never ask the user to confirm a plain-text draft or config_validation_failed result. After one successful config_draft, stop tool calls and ask the user to confirm or cancel.",
