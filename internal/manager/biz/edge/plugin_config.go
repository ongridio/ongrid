...
// defaultMetricsTargetURLs matches internal/edgeagent/plugins/metrics/scrape.go
// defaultURLs. When gpumetrics sync creates the first metrics-plugin DB row,
// seed these so the edge keeps scraping host/process exporters instead of
// switching to GPU-only.
var defaultMetricsTargetURLs = []string{
...
func (uc *PluginConfigUC) syncGPUTargetURL(ctx context.Context, edgeID uint64, gpuSpec map[string]interface{}) error {
	...
	if len(urls) == 0 {
		// No explicit scrape list yet — mirror the edge metrics plugin's
		// implicit defaults before appending the GPU exporter. Without
		// this, a fresh metrics row would scrape GPU only and drop
		// node_cpu_* / process_* series from Prometheus.
		urls = append([]string(nil), defaultMetricsTargetURLs...)
	}
	for _, u := range urls {
		if u == url {
