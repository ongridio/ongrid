...
//   - watchUpgradeHealth → Machine.HealthPoll (HealthPoll redesign: renamed from HealthCheck)
...
// 用于断言被测代码是否输出了期望的 warn 日志（CLAUDE.md §10 显式失败原则）。
...
// --- Machine.HealthPoll ---
// HealthPoll redesign (workerCtx cascade + file lock fix)：HealthPoll 替代原 HealthCheck，移除 timer
// 分支（workerCtx cascade root cause）+ RollbackAndMark 调用（file lock root cause — 改在 superviseWorker
// 循环顶部 worker 已退出空窗执行）。HealthPoll 仅 polling + IsUpgradeHealthy +
// CleanupPrevious + 清 awaiting_health sentinel（成功路径）。
// 已删除测试（timer 路径已移除）：
//   - TestMachine_HealthCheck_TimeoutRollback — rollback 决策移到 superviseWorker
//   - TestMachine_HealthCheck_Timeout_ClearsAwaitingSentinel — sentinel 清理移到
//     superviseWorker 循环顶部（worker_test.go 覆盖）
...
// --- async health arming：BootCheck supervisor_selfswap_awaiting_health sentinel ---
// BootCheck deadlock 修复：self-swap 完成后，BootCheck 步骤 1 不立即 rollback，
// 而是检测 awaiting_health sentinel → arm HealthCheck（设 pendingHealthCheck=true），
// 让 superviseWorker 启 worker 后用 HealthCheck 180s grace 确认健康。

// TestMachine_BootCheck_SelfSwapAwaitingHealth_ArmsHealthCheck 验证 async health arming 分支：
...
func TestMachine_BootCheck_SelfSwapAwaitingHealth_ArmsHealthCheck(t *testing.T) {
	...
	writeTestFile(t, filepath.Join(binDir, "worker.exe"), "new-supervisor-swap")
	...
	// async health arming：应 arm HealthCheck
	if !m.PendingHealthCheck() {
		t.Error("PendingHealthCheck() = false, want true (async health arming 应 arm HealthCheck)")
	}

	// rollback.done 不应存在（未 rollback）
	if _, err := os.Stat(filepath.Join(stageDir, RollbackDoneFile)); err == nil {
		t.Error("rollback.done sentinel 不应存在（async health arming 跳过 rollback）")
	}

	// worker.exe 不应被回滚（仍是新版本）
	got, _ := os.ReadFile(filepath.Join(binDir, "worker.exe"))
	if string(got) != "new-supervisor-swap" {
		t.Errorf("worker.exe = %q, want %q (async health arming 不应 rollback)",
...
// 清除 supervisor_selfswap_awaiting_health sentinel（async health arming sentinel 生命周期右端点
...
func TestMachine_HealthPoll_Success_ClearsAwaitingSentinel(t *testing.T) {
	...
	writeTestFile(t, filepath.Join(stageDir, HealthyMarkerFile), "v1.0.0")
	writeTestFile(t, filepath.Join(binDir, "worker.exe.previous"), "old")
	// awaiting sentinel 存在（async health arming BootCheck 留下的）
	writeTestFile(t, filepath.Join(stageDir, SupervisorSelfSwapAwaitingHealthFile), "awaiting")
