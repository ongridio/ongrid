...
#   K8S_EDGE_IMAGE_TAG VERSION by default
#
...
K8S_EDGE_IMAGE_TAG="${K8S_EDGE_IMAGE_TAG:-${VERSION}}"
...
if [[ -d "${REPO_ROOT}/deploy/kubernetes/ongrid-edge" ]]; then
    ...
    bash "${REPO_ROOT}/dist/package-k8s-chart.sh" \
        "${REPO_ROOT}/deploy/kubernetes/ongrid-edge" \
        "${STAGE_DIR}/edge/k8s/ongrid-edge.tgz" \
        "${VERSION}" \
        "${K8S_EDGE_IMAGE_TAG}"
    cp "${STAGE_DIR}/edge/k8s/ongrid-edge.tgz" "${STAGE_DIR}/edge/ongrid-edge.tgz"
    log "  + edge/k8s/ongrid-edge.tgz (version=${K8S_EDGE_IMAGE_TAG})"
else
