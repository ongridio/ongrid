...
#   K8S_EDGE_IMAGE_PLATFORM linux/amd64 by default for Kubernetes node pulls
...
# commands generated by pre-k8s-path managers.
if [[ -d "${REPO_ROOT}/deploy/kubernetes/ongrid-edge" ]]; then
    mkdir -p "${STAGE_DIR}/edge/k8s"
    CHART_STAGE="${STAGE_DIR}/.ongrid-edge-chart"
    rm -rf "${CHART_STAGE}"
    mkdir -p "${CHART_STAGE}"
    cp -R "${REPO_ROOT}/deploy/kubernetes/ongrid-edge" "${CHART_STAGE}/ongrid-edge"
    awk -v app_version="${K8S_EDGE_IMAGE_TAG}" '
        /^appVersion:/ { print "appVersion: \"" app_version "\""; next }
        { print }
    ' "${CHART_STAGE}/ongrid-edge/Chart.yaml" > "${CHART_STAGE}/ongrid-edge/Chart.yaml.tmp"
    mv "${CHART_STAGE}/ongrid-edge/Chart.yaml.tmp" "${CHART_STAGE}/ongrid-edge/Chart.yaml"
    COPYFILE_DISABLE=1 tar -C "${CHART_STAGE}" -czf "${STAGE_DIR}/edge/k8s/ongrid-edge.tgz" ongrid-edge
    rm -rf "${CHART_STAGE}"
    cp "${STAGE_DIR}/edge/k8s/ongrid-edge.tgz" "${STAGE_DIR}/edge/ongrid-edge.tgz"
    log "  + edge/k8s/ongrid-edge.tgz (appVersion=${K8S_EDGE_IMAGE_TAG})"
else
