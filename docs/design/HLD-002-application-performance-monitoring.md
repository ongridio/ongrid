...
- 服务列表按窗口聚合请求率、错误率和延迟桶，接口排行增加 span_name。SERVER 与 CONSUMER 分开。
...
APM 告警模板由后端使用与页面相同的查询构造生成，明确 SERVER/CONSUMER、最小观测流量和持续窗口，用户在现有规则编辑器预览/保存。模板不自动启用业务告警，不改变已有 Trace 规则。
