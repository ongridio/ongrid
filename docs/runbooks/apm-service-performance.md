...
服务列表默认查询全部 HTTP/RPC 应用指标，按服务名、环境、服务命名空间分组；同一服务仅列一次，协议及其指标逐行对齐。环境与命名空间位于搜索框前。点击表头排序：RPS 按服务总速率，错误率按请求加权的服务整体比例，P95 按各协议中的最高值排序，页面不虚构跨协议 P95。点击服务默认打开该服务已观测到的首个协议，点击协议可直接进入对应详情。在「接入管理」选择旧版指标格式或 Trace 样本来源；Trace 样本视图单独选择 SERVER/CONSUMER。打开服务后，概览集中展示 RED、重点接口与上下游；延迟默认 P95，可切换 P50/P99。调用链沿用绝对时间与完整服务身份；运行时与按需 pprof 在「实例」，接入诊断在「接入管理」，请求告警由顶部「创建告警」进入原规则编辑器。
