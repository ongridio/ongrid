...
这些数量不是全量失败请求，也不是故障历史首次发生时间。达到采样上限或部分 Trace 详情读取失败会显示不完整提示；全部详情失败显示查询错误。聚合以当前窗口快照展示，点击顶部刷新更新，不自动每 30 秒重复读取链路详情。单个 Trace 详情最多 2 MiB，同时最多读取 4 条，遵循现有 APM 请求超时和并发限制。
