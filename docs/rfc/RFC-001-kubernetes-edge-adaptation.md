...
Event 当前快照在 controller 下一轮上报时按快照范围收敛删除；manager 额外执行硬保留策略，默认按 Kubernetes 事件时间保留 24h、每集群最多 5000 条、每小时清理一次（`ONGRID_K8S_EVENT_RETENTION` / `ONGRID_K8S_EVENT_MAX_PER_CLUSTER` / `ONGRID_K8S_EVENT_CLEANUP_INTERVAL`）。这里不长期保存无限 Event 历史，后续需要审计级事件历史再迁移到 Loki / ClickHouse。
